...
def build_consonant_single(config):
    extractor = PoseExtractor(config)

    consonant_root = Path(config['paths']['raw_data']) / "NSL_Consonant_Part_1"
    base_out_dir = Path(config['paths']['sequences_dir'])
    
    metadata = []
    
    if not consonant_root.exists():
        print(f"Error: Root folder {consonant_root} does not exist.")
        return []

    for folder in consonant_root.iterdir():
        if not folder.is_dir(): continue
        
        if not (folder.name.startswith("S1_") or folder.name.startswith("S2_")):
            continue
        
        target_subfolder = base_out_dir / "NSL_Consonant_Part_1" / folder.name
        target_subfolder.mkdir(parents=True, exist_ok=True)
        
        is_cropped = any(cid in folder.name for cid in config['processing']['cropped_identifiers'])
        print(f"\Processing Consonant Folder: {folder.name}")
        ...
        for video_path in folder.glob("*.MOV"):
            ...
            nepali_char = config['processing']['consonant_label_map'].get(raw_label, "Unknown")
            ...
            metadata.append({
                'relative_path': str(Path("sequences") / "NSL_Consonant_Part_1" / folder.name / f"{raw_label}.npz"),
                'char': nepali_char,
